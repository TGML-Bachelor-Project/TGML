...
def animate_nomodel(z0, v0, time_deltas, step_size, start_times, t_start, t_end, num_of_time_points, device):
    ...
    fig.show()
    fig.write_html(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'animations', 'latest_animation_resistance.html'), auto_play=False)
...
def animate_nomodel_resistance(z0, v0, time_deltas, step_size, num_of_steps, t_start, t_end, num_of_time_points, device):
    # Starting positions for each model step
    steps_z0 = z0.unsqueeze(2) + torch.cumsum(v0*time_deltas, dim=2)
    steps_z0 = torch.cat((z0.unsqueeze(2), steps_z0), dim=2)
    ...
    df = pd.DataFrame({
        'node': ['P'+str(n) for n in [*list(range(step_zt.shape[0]))]*len(times)],
        # 'node': ['Computer', 'P1','P2','P3','P4','P5','P6','P7','P8'],
        'x': step_zt[:,0,:].T.flatten().tolist(),
        'y': step_zt[:,1,:].T.flatten().tolist(),
        't': [t for t in times.tolist() for _ in list(range(step_zt.shape[0]))]
    })

    fig = px.scatter(df, x='x', y='y', animation_frame='t', animation_group='node', color="node", color_discrete_sequence=px.colors.qualitative.Light24,
               log_x=False, size_max=20,
               range_x=[torch.min(step_zt[:,0,:]).item(), torch.max(step_zt[:,0,:]).item()], 
               range_y=[torch.min(step_zt[:,1,:]).item(), torch.max(step_zt[:,1,:]).item()])
    fig.layout.updatemenus[0].buttons[0].args[1]['frame']['duration'] = 100
    ...
    fig.show()
    fig.write_html('/home/augustsemrau/drive/bachelor/TGML/animations_rl_data/latest_animation_resistancePositionCorrection.html', auto_play=False)
    # fig.write_html('/home/augustsemrau/drive/bachelor/TGML/animations_rl_data/lyon_full_lr0025_steps240.html', auto_play=False)
