...
if __name__ == '__main__':
    ...
    if vectorized != 2: 
        ...
        if isinstance(model_beta, np.ndarray):
            ...
            gt_model = GTMultiBetaStepwise(n_points=num_nodes, z=torch.from_numpy(z0), v=v0.detach().clone(), beta=torch.tensor([true_beta]*num_steps), 
            ...
            result_model = GTStepwiseConstantVelocityModel(n_points=num_nodes, z=result_z0, v=result_v0, beta=result_beta,
                                                            steps=num_steps, max_time=max_time, device=device)
            gt_model = GTStepwiseConstantVelocityModel(n_points=num_nodes, z=torch.from_numpy(z0), v=v0.detach().clone(), beta=torch.tensor([true_beta]*num_steps), 
                                                            steps=v0.shape[2], max_time=max_time, device=device)
