...
from utils.integrals.analytical import stepwise_analytical_integral as evaluate_integral
from utils.integrals.analytical import vec_analytical_integral as old_evaluate_integral
...
class StepwiseVectorizedConstantVelocityModel(nn.Module):
    ...
    def old_steps(self, times:torch.Tensor) -> torch.Tensor:
        ...
        '''
        Zt = self.z0 + self.v0[:,:,0] * times.unsqueeze(1).unsqueeze(1)
        return Zt
    ...
    def steps(self, times:torch.Tensor) -> torch.Tensor:
        ...
        time_step_delta_difs = (times-time_step_floored*self.time_delta_size) #Don't take last delta, as we do not step from last
        time_step_indices = time_step_floored.tolist() #Don't take last step
    ...
    def forward(self, data:torch.Tensor, t0:torch.Tensor, tn:torch.Tensor) -> torch.Tensor:
        ...
        # old_event_intensity = torch.sum(torch.sum(old_log_intensities, dim=0).triu(diagonal=1))
        # old_non_event_intensity = torch.sum(old_evaluate_integral(t0, tn, 
