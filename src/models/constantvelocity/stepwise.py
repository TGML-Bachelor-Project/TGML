...
class StepwiseVectorizedConstantVelocityModel(nn.Module):
    ...
    def forward_copy(self, data:torch.Tensor, t0:torch.Tensor, tn:torch.Tensor) -> torch.Tensor:
        '''
        ...
        unique_times, unique_time_counts = torch.unique(data[:,2], return_counts=True)
        unique_time_batches = (torch.split(unique_times, self.time_batch_size) 
                                if self.time_batch_size > 0 else unique_times)
        unique_time_count_batches = (torch.split(unique_time_counts, self.time_batch_size) 
                                        if self.time_batch_size > 0 else unique_time_counts)

        # Batching by time
        current_time_index = 0
        for batch_id in range(len(unique_time_batches)):
            time_batch = unique_time_batches[batch_id]
            counts = unique_time_count_batches[batch_id]
            # Use counts of unique time points to calculate current data batch end index
            batch_end_time_index = current_time_index + torch.sum(counts)
            # Compute the indices of each unique time point in the batch
            _, batch_time_indices = torch.unique(data[current_time_index:batch_end_time_index,2], return_inverse=True)

            # Batching by nodes
            # Sorting node pairs to avoid duplicates in unique e.g. both [1,3] and [3,1]. We only want one of them
            batch_node_pairs, _ = torch.sort(data[current_time_index:batch_end_time_index, :2].long(), dim=0)
            unique_node_pairs, unique_node_indices = torch.unique(batch_node_pairs, dim=0, return_inverse=True)
            nodes = torch.unique(unique_node_pairs)
            node_batch_size = self.node_pair_batch_size if self.node_pair_batch_size > 0 else unique_node_pairs.shape[0]
            node_batches = torch.split(nodes, node_batch_size)

            for node_batch in node_batches:
                nodes, node_indices = torch.unique(node_batch, return_inverse=True)
                node_time_indices = unique_node_indices[unique_node_indices in node_indices]
                
                log_intensities = self.log_intensity_function(nodes, times=time_batch)
                event_intensity += torch.sum(log_intensities[node_batch[unique_node_indices,0], 
                                            node_batch[unique_node_indices,1], batch_time_indices]) 

            current_time_index += batch_end_time_index
    ...
    def forward(self, data:torch.Tensor, t0:torch.Tensor, tn:torch.Tensor) -> torch.Tensor:
        ...
        '''
        event_intensity = torch.tensor(0.)
        unique_node_pairs, _ = torch.sort(torch.unique(data[:,:2], dim=0), dim=1)
        node__pair_splits = torch.split(unique_node_pairs, self.node_pair_batch_size)

        # Batching in node dimension
        for node_pairs in node__pair_splits:
            unique_nodes = torch.unique(node_pairs).long()
            num_of_batch_nodes_in_nodes = torch.sum(data[:,:2] == node_pairs.unsqueeze(1), dim=2)
            # == 2 because both node[0] and node[1] must be in the chosen nodes
            batch_data = data[torch.sum(num_of_batch_nodes_in_nodes == 2, dim=0)] 
            # batch_data = data[torch.isin(data[:,:2], node_pairs)]
            unique_times = torch.unique(batch_data[:,2])
            time_splits = torch.split(unique_times, self.time_batch_size)

            # Batching in time dimension
            for times in time_splits:
                time_mask = torch.sum(batch_data[:,2] == times.unsqueeze(1), dim=0)
                i = batch_data[time_mask][:,0].long()
                j = batch_data[time_mask][:,1].long()
                log_intensities = self.log_intensity_function(unique_nodes, times=times)
